...
private const val PERMANETLY_DELETE_DIALOG = 3
...
fun TrashScreen(viewModel: MainViewModel){
    ...
    Scaffold(
        topBar = {
            val areActionVisible = selectedNotes.isNotEmpty()
            TrashTopAppBar(
                onNavigationIconClick = {
                    coroutineScope.launch { scaffoldState.drawerState.open() }
                },
                onRestoreNotesClick = {dialogState.value = RESTORE_NOTES_DIALOG},
                onDeleteNotesClick = {dialogState.value=PERMANETLY_DELETE_DIALOG},
                areActionsVisible=areActionVisible
            )
        },
...
private fun TrashTopAppBar(
    ...
    TopAppBar(
        title = { Text(text = "Trash Demo", color= MaterialTheme.colors.onPrimary)},
        navigationIcon = {
...
private fun Content(
    ...
    Column {
        ...
        val filteredNotes = when (selectedTab) {
            ...
            else -> throw illegalDecoyCallException("Tab not supported - index: $selectedTab")
        }
        LazyColumn {
...
private fun mapDialogTitle(dialog: Int): String = when (dialog){
    ...
    PERMANETLY_DELETE_DIALOG->"Delete notes forever"
...
private fun mapDialogText(dialog: Int):String=when(dialog){
    ...
    PERMANETLY_DELETE_DIALOG->"Are you sure you want to delete selected notes permanently?"
    else->throw RuntimeException("Dialog not supported:$dialog")
}